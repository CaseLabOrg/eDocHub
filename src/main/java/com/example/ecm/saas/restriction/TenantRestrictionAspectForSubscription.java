...
public class TenantRestrictionAspectForSubscription {
    private final SubscriptionRepository subscriptionRepository;

    /**
     * Проверяет доступ к ресурсу Subscription.
     *
    ...
    public Object checkTenantAccess(ProceedingJoinPoint joinPoint, TenantRestrictedForSubscription tenantRestricted) throws Throwable {
    ...
    private Long getResourceTenantId(Object[] args) {
        if (args.length > 0 && args[0] instanceof Long id) {
            Subscription subscription = subscriptionRepository.findById(id)
                    .orElseThrow(() -> new NotFoundException("Subscription не найден для ID: " + id));
            if (subscription.getTenant() != null) {
                return subscription.getTenant().getId();
            } else {
                throw new NotFoundException("У Организации нет такой подписки");
            }
        }
        throw new IllegalArgumentException("Некорректный аргумент: ID подписки отсутствует.");
    }
}
